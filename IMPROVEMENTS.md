...
- **Local dev DB**: Postgres runs in Docker (`bun run db:up`, defined in `docker-compose.yml`), `DATABASE_URL` in `.env` points at it. Seed with `bun run db:seed` → user `Sam` / password `password`, household `Testhuishouden`. Migrations: `bunx prisma migrate diff --from-url "$DATABASE_URL" --to-schema-datamodel src/lib/db/schema.prisma --script` into a new folder under `src/lib/db/migrations/`, then `bunx prisma migrate deploy` (plain `prisma migrate dev` fails in non-interactive shells). Never point `DATABASE_URL` at the Neon URL commented in `.env`.
...
7. **Thumb-zone ergonomics (user, 2026-07-06: "why are the buttons now all at the top, its a phone app, peoples thumbs cant reach")**: frequent controls must live at the BOTTOM. Move the Huishouden/Persoonlijk segmented control back into the footer (below the add-bar, like the original app). Footer stack top-to-bottom: "n in je mandje/Wissen" bar → add-bar → segmented control → safe-area padding. Replace the top-right "Categorie toevoegen" button with a quiet ghost row at the END of the list ("+ Categorie toevoegen", opens the same dialog) — rare action, doesn't need prime space, but must not require reaching the top. Header keeps only sign-out/info (rare actions are fine up top).
